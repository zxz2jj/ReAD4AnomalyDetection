...
def get_pcs_score(dataset_name, checkpoint_path, pictures_classified):

    model = tf.keras.models.load_model(checkpoint_path+'tf_model.h5')
    pcs_uncertainty_dict = {}
    for category in range(num_of_labels[dataset_name]):
        pcs_uncertainty_category = {}
        ...
        if correct_pictures.shape[0] != 0:
            batch_size = 128
            pcs_uncertainty = list()
            for i in range(0, len(correct_pictures), batch_size):
                batch = correct_pictures[i:i + batch_size]
                batch_output = model([batch]).numpy()
                index = np.argpartition(batch_output, -2)[:, -2:]
                top_2_scores = np.take_along_axis(batch_output, index, axis=-1)
                batch_pcs_uncertainty = abs(top_2_scores[:, 0] - top_2_scores[:, 1])
                pcs_uncertainty.append(1-batch_pcs_uncertainty)
            pcs_uncertainty_category['correct_pictures'] = np.concatenate(pcs_uncertainty)
            pcs_uncertainty_category['correct_prediction'] = pictures_classified[category]['correct_prediction']
        else:
            pcs_uncertainty_category['correct_pictures'] = np.array([])
            pcs_uncertainty_category['correct_prediction'] = np.array([])
        if wrong_pictures.shape[0] != 0:
            ...
            for i in range(0, len(wrong_pictures), batch_size):
                ...
                batch_pcs_uncertainty = abs(top_2_scores[:, 0] - top_2_scores[:, 1])
                pcs_uncertainty.append(1-batch_pcs_uncertainty)
            pcs_uncertainty_category['wrong_pictures'] = np.concatenate(pcs_uncertainty)
            pcs_uncertainty_category['wrong_prediction'] = pictures_classified[category]['wrong_prediction']
        else:
            pcs_uncertainty_category['wrong_pictures'] = np.array([])
            pcs_uncertainty_category['wrong_prediction'] = np.array([])

        pcs_uncertainty_dict[category] = pcs_uncertainty_category

    return pcs_uncertainty_dict
...
if __name__ == "__main__":
    ...
    # test_confidence = get_pcs_score(dataset_name=id_dataset, checkpoint_path=model_path,
    ...
    #     ood_confidence = get_pcs_score(dataset_name=id_dataset, checkpoint_path=model_path,
    ...
    for attack in adversarial_attacks:
        ...
        print('\nGet neural value:')
        print('CLEAN dataset ...')
        clean_data_confidence = get_pcs_score(dataset_name=clean_dataset, checkpoint_path=model_path,
                                              pictures_classified=clean_data_classified)
        print('ADVERSARIAL dataset ...')
        adv_data_confidence = get_pcs_score(dataset_name=clean_dataset, checkpoint_path=model_path,
