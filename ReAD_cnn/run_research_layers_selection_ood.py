...
def get_neural_value(id_dataset, layers, tf_model_path, pictures_classified):
    ...
    get_layers_output = tf.keras.backend.function(inputs=model.layers[0].input,
                                                  outputs=[model.layers[layer].output for layer in layers])
    ...
    for category in range(num_of_labels[id_dataset]):
        ...
        if correct_pictures is not None:
            layers_output = get_layers_output([correct_pictures])
            correct_layers_output[category] = layers_output
        else:
            correct_layers_output[category] = None
        if wrong_pictures is not None:
            layers_output = get_layers_output([wrong_pictures])
            wrong_layers_output[category] = layers_output
        else:
...
if __name__ == "__main__":
    ...
    if os.path.exists(f'./data/mnist/single_layer_selection.pkl') and \
            os.path.exists(f'./data/mnist/multi_layers_selection.pkl') and \
            os.path.exists(f'./data/fmnist/single_layer_selection.pkl') and \
            os.path.exists(f'./data/fmnist/multi_layers_selection.pkl') and \
            os.path.exists(f'./data/cifar10/single_layer_selection.pkl') and \
            os.path.exists(f'./data/cifar10/multi_layers_selection.pkl'):

        # # Single Layer
        print('Single Layer Selection Analyzing...')
        x = [a for a in range(1, 8)]
        x_cifar = [3, 4]
        file1 = open(f'./data/mnist/single_layer_selection.pkl', 'rb')
        single_layer_performance_1 = pickle.load(file1)
        file3 = open(f'./data/fmnist/single_layer_selection.pkl', 'rb')
        single_layer_performance_2 = pickle.load(file3)
        file4 = open(f'./data/cifar10/single_layer_selection.pkl', 'rb')
        single_layer_performance_3 = pickle.load(file4)

        MNISTvsFMNIST = single_layer_performance_1['FMNIST']
        MNISTvsOmniglot = single_layer_performance_1['Omniglot']
        FMNISTvsMNIST = single_layer_performance_2['MNIST']
        FMNISTvsOmniglot = single_layer_performance_2['Omniglot']
        Cifar10vsTinyImageNet = single_layer_performance_3['TinyImageNet']
        Cifar10vsiSUN = single_layer_performance_3['iSUN']
        Cifar10vsLSUN = single_layer_performance_3['LSUN']
        ...
        plt.plot(x, MNISTvsFMNIST, label='MNIST vs. FMNIST', marker='.')
        plt.plot(x, MNISTvsOmniglot, label='MNIST vs. Omniglot', marker='.')
        plt.plot(x, FMNISTvsMNIST, label='FMNIST vs. MNIST', marker='.')
        plt.plot(x, FMNISTvsOmniglot, label='FMNIST vs. Omniglot', marker='.')
        plt.plot(x_cifar, Cifar10vsTinyImageNet, label='Cifar10 vs TinyImageNet', marker='.')
        plt.plot(x_cifar, Cifar10vsiSUN, label='Cifar10 vs iSUN', marker='.')
        plt.plot(x_cifar, Cifar10vsLSUN, label='Cifar10 vs. LSUN', marker='.')
        ...
        x = [a for a in range(1, 8)]
        x_cifar = [4, 5]
        file4 = open(f'./data/mnist/multi_layers_selection.pkl', 'rb')
        multi_layers_performance_1 = pickle.load(file4)
        file5 = open(f'./data/fmnist/multi_layers_selection.pkl', 'rb')
        multi_layers_performance_2 = pickle.load(file5)
        file6 = open(f'./data/cifar10/multi_layers_selection.pkl', 'rb')
        multi_layers_performance_3 = pickle.load(file6)

        MNISTvsFMNIST = multi_layers_performance_1['FMNIST']
        MNISTvsOmniglot = multi_layers_performance_1['Omniglot']
        FMNISTvsMNIST = multi_layers_performance_2['MNIST']
        FMNISTvsOmniglot = multi_layers_performance_2['Omniglot']
        Cifar10vsTinyImageNet = multi_layers_performance_3['TinyImageNet']
        Cifar10vsiSUN = multi_layers_performance_3['iSUN']
        Cifar10vsLSUN = multi_layers_performance_3['LSUN']
    ...
    #  else run research on each dataset ...
    id_dataset_name = 'mnist'
    # id_dataset_name = 'fmnist'
    # id_dataset_name = 'cifar10'

    # single layer selection
    if os.path.exists(f'./data/{id_dataset_name}/single_layer_selection.pkl'):
        print('Single Layer Selection Research has finished! The result was saved in:\n '
              f'./data/{id_dataset_name}/single_layer_selection.pkl')
    else:
        ...
        for i in range(len(research_layers)):
            ...
            for ood in OOD_dataset:
                ...
                ood_distance = statistic_distance(id_dataset=id_dataset_name,
                                                  ...
                                                  cluster_centers=k_means_centers)
                auc = sk_auc(test_distance, ood_distance, num_of_labels[id_dataset_name])
                single_layer_performance[ood].append(auc)
                print('\nPerformance of Detector:')
                print('AUROC: {:.6f}'.format(auc))
                print('*************************************\n')
        single_performance_file = open(f'./data/{id_dataset_name}/single_layer_selection.pkl', 'wb')
    ...
    # layers aggregation
    if os.path.exists(f'./data/{id_dataset_name}/multi_layers_selection.pkl'):
        print('Layers Aggregation Selection Research has finished! The result was saved in:\n '
              f'./data/{id_dataset_name}/multi_layers_selection.pkl')
    else:
        if id_dataset_name == 'mnist':
            model_path = './models/lenet_mnist/'
            research_layers = [-15, -14, -11, -10, -8, -6, -4]
        ...
        for i in range(len(research_layers)):
            ...
            for ood in OOD_dataset:
                ...
                auc = sk_auc(test_distance, ood_distance, num_of_labels[id_dataset_name])
                multi_layers_performance[ood].append(auc)
                print('\nPerformance of Detector:')
                print('AUROC: {:.6f}'.format(auc))
                print('*************************************\n')

        multi_performance_file = open(f'./data/{id_dataset_name}/multi_layers_selection.pkl', 'wb')
