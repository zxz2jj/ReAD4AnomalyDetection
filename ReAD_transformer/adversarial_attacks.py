...
class AttackEval:
    ...
    Generator[Dict[str, Any], None, None]:
        ...
        if num_workers > 0:
            ...
            with ctx.Pool(num_workers, initializer=worker_init,
                          initargs=(self.attacker, self.victim, self.invoke_limit)) as pool:
                for ret in self.__iter_metrics(
                        zip(dataset, pool.imap(worker_process, self.__iter_dataset(dataset), chunksize=chunk_size))):
                    yield ret
    ...
    def eval(self, dataset: Iterable[Dict[str, Any]], total_len: Optional[int] = None, visualize: bool = False,
             progress_bar: bool = False, num_workers: int = 0, chunk_size: Optional[int] = None):
        """
        ...
        for i, res in enumerate(result_iterator):
            ...
            if visualize and (TAG_Classification in self.victim.TAGS):
                ...
                if res["success"]:
                    x_adv = res["result"]
                    attack_result['clean'].append(x_orig)
                    attack_result['adversarial'].append(x_adv)
                    if Tag("get_prob", "victim") in self.victim.TAGS:
                        ...
                        y_orig = probs[0]
                        y_adv = probs[1]
                        attack_result['label'].append(y_orig.argmax())
                        attack_result['clean_confidence'].append(y_orig.max() * 100)
                        attack_result['target'].append(y_adv.argmax())
                        attack_result['adv_confidence'].append(y_adv.max() * 100)
